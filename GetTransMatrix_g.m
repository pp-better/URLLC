function [ P_out ] = GetTransMatrix_g( n, M, flag, noise, P_UE0, sinr_th, para1, para2, delta, pa_max )
...
%         p_a = min(1, para1*M/(residual_n+1)+para2);
        p_a = min(pa_max, para1*M/(residual_n+1)+para2);
%         if p_a >= 1
%             p_a = M/(M+1);
%         end
    else
    ...
    if p_a==1
        p_sinr_lower = gammainc(sinr_th*(noise+P_UE0*delta^2*residual_n)/(P_UE0*(delta^2+1)),M-residual_n+1);
...
%             g_max = residual_n;
            for g = new_succ:g_max
                p_sinr_lower = gammainc(sinr_th*(noise+P_UE0*delta^2*g)/(P_UE0*(delta^2+1)),M-g+1);
                p_sinr_upper = 1-p_sinr_lower;
                if p_sinr_upper == 1
